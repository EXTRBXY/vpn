...
{
    private readonly IAppUpdateController _controller; private readonly IInstallerUpdateService _installer; private readonly IInstallerLaunchService _launcher; private readonly AppStateModel _state; private readonly Action _exit;
    private AppReleaseModel? _release; private string _status=""; private int _progress;
    public UpdateViewModel(IAppUpdateController controller,IInstallerUpdateService installer,IInstallerLaunchService launcher,AppStateModel state,Action exit)
    { _controller=controller;_installer=installer;_launcher=launcher;_state=state;_exit=exit;CheckCommand=new AsyncRelayCommand(CheckAsync);InstallCommand=new AsyncRelayCommand(InstallAsync,()=>_release is not null); }
    ...
    public async Task CheckAsync(){Status="Проверка…";var result=await _controller.CheckAsync(_state,GetVersion());_release=result.AvailableRelease;Status=!result.Succeeded?"Не удалось проверить обновления.":_release is null?"Установлена актуальная версия.":$"Доступна версия {_release.Semver}.";(InstallCommand as AsyncRelayCommand)?.RaiseCanExecuteChanged();}
    public async Task CheckAtStartupAsync(){_installer.CleanupOldInstallers();_controller.RecordInstalledVersion(_state,GetVersion());if(_controller.IsPeriodicCheckDue(_state,DateTimeOffset.UtcNow))await CheckAsync();}
